...
ブラウザで `http://localhost:8080` にアクセスしてください。インストーラーが始まります。

DBは、デフォルトで `mtdb` というdbができているので、それが利用出来ます。rootユーザー、パスワードはなしでOKです。
(必要に応じて、自分でデータベースやパスワードを設定することも可能です。その場合は、コンテナにログインしてmysqlコマンドで設定して下さい)
...
- 1.4 For Windows, share directory is /var/mt.
