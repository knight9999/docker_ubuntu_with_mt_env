...
とすれば、イメージファイルの作成をスキップしてMySQL用のボリュームの用意に進んでください。
...
Windowsであれば、ドライブから始まって次のようにします。また、ディレクトリはマウントしてもWindowsではpermissionの都合から書き込みが出来ないため、マウントしません。その代わり、Sambaでマウントします。

```
> docker run --privileged -d --name mt_server -p 8022:22 -p 8080:80 -p 139:139 -p 445:445 -v mt-storage:/var/db/mysql  -it ubuntu_with_mt_env
```
